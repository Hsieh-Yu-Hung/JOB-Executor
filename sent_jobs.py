...
def get_logs_and_delete_job(job_name):
    ...
    time_filter = f'timestamp>="{current_time}"'

    # 使用時間範圍過濾 log
    os.system(f"""gcloud logging read 'resource.type = "cloud_run_job" resource.labels.job_name = "{job_name}" resource.labels.location = "{args.region}" labels."run.googleapis.com/task_index" = "0" severity>=DEFAULT {time_filter}' | grep 'textPayload' > {os.path.join("logs", f"{job_name}.logs")}""")
    
    os.system(f"gcloud run jobs delete {job_name} --region {args.region} --quiet")
    
    # 讀取 log 並判斷 job 是否成功或失敗
    with open(os.path.join("logs", f"{job_name}.logs"), "r") as file:
        logs = file.read()
        if "失敗" in logs:
            return False
        else:
